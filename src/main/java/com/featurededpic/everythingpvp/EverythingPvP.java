...
public class EverythingPvP extends JavaPlugin {
	...
	public boolean onCommand(CommandSender s, Command c, String l, String[] args) {
		if(c.getName().equalsIgnoreCase("bloodcolor")){
			if(s.isOp() || s.hasPermission("pvp.bloodcolor.change")){
				if(args.length == 0){
		...
		if(c.getName().equalsIgnoreCase("pvp")){
			if(args.length != 1){
				ChatManager.getInstance().error("Incorrect arguments! To toggle the Server's PvP, use /pvp server. More options will arrive soon!", s);
				return true;
			}
			...
			if(s.isOp() || s.hasPermission("pvp.pvp.server")){
				if(SettingsManager.getInstance().getSettings().getString("ServerPvP").equalsIgnoreCase("true")){
					...
					SettingsManager.getInstance().saveSettings();
					SettingsManager.getInstance().reload();
					ChatManager.getInstance().info("Set the Server-Wide PvP to FALSE!", s);
					return true;
				}
				else {
					...
					SettingsManager.getInstance().saveSettings();
					SettingsManager.getInstance().reload();
					ChatManager.getInstance().info("Set the Server-Wide PvP to TRUE!", s);
					return true;					
				}
			}
			ChatManager.getInstance().error("You do not have permission to do that!", s);
		...
		if(c.getName().equalsIgnoreCase("blood")){
			if(s.isOp() || s.hasPermission("pvp.blood.change")){
				if(SettingsManager.getInstance().getSettings().getString("BloodVisible").equalsIgnoreCase("true")){
					...
					SettingsManager.getInstance().saveSettings();
					SettingsManager.getInstance().reload();
					ChatManager.getInstance().info("Set the Blood Visbility to FALSE!", s);
					return true;
				}
				else {
					...
					SettingsManager.getInstance().saveSettings();
					SettingsManager.getInstance().reload();
					ChatManager.getInstance().info("Set the Blood Visbility to TRUE!", s);
					return true;					
				}
			}
			ChatManager.getInstance().error("You do not have permission to do that!", s);
